...
// Define API_URL from environment variables
...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const checkReportStatus = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await fetch(`${API_URL}/api/questionnaire/report-status`, {
          headers: { 
            "Authorization": `Bearer ${token}`,
          },
        });
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = reportPath; // Use reportPath as the filename
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(url);
  ...
  // Helper function to determine if report is available
  const isReportAvailable = status === "Report Generated" || status === "Final Report Successful" || status === "Successful";
            ...
            <div className="space-y-6">
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                    status === "Analyzing" ? "bg-yellow-100 text-yellow-800" :
                    status === "Successful" ? "bg-blue-100 text-blue-800" :
                    status === "Report Generated" ? "bg-green-100 text-green-800" :
                    "bg-gray-100 text-gray-800"
                  }`}>
                    {status}
                  </span>
              ...
              {reportPath && (
                <button 
                  ...
                  className={`w-full inline-flex items-center justify-center px-6 py-4 border border-transparent text-base font-medium rounded-xl text-white bg-gradient-to-r from-indigo-600 to-indigo-700 shadow-lg transform transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ${
                    isReportAvailable 
                      ? 'hover:from-indigo-700 hover:to-indigo-800 hover:shadow-xl hover:scale-[1.02]' 
